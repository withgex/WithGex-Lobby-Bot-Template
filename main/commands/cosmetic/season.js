...
module.exports = {
  ...
  run: async (client, bot, message, args, getCosmetic) => {
    let cid; let skin;
    const season = args[0];
    if (!season) return error2('❌│ Proporcione una temporada', message);
    ...
    switch (season) {
      ...
      default: {
        return error2('Elija entre una temporada * válida *, por ejemplo 10', message);
      }
    }

    bot.party.me.setOutfit(cid);
    success3(`La skin se ha configurado para **${skin}** [**${cid}**], Temporada ${season}'s skin de nivel máximo.`, message);
  }
}