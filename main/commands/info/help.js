...
module.exports = {
  ...
  run: async (client, bot, message, args, getCosmetic) => {
    ...
    const commands = {
      ...
      skins: client.commands.filter(x => x.category === 'skins'),
      bailes: client.commands.filter(x => x.category === 'bailes'),
    ...
    .addField(`‣  Skins - Peaux [${commands.skins.size}]`, commands.skins.map(x => `\`${x.name}\``).join(', '))
    .addField(`‣  Bailes - Emotes - Danses [${commands.bailes.size}]`, commands.bailes.map(x => `\`${x.name}\``).join(', '))
    .addField(`‣  Info [${commands.info.size}]`, commands.info.map(x => `\`${x.name}\``)
    .join(', '))
    .setFooter(`Gex Lobby Bot - dsc.gg/gex`, 'https://cdn.discordapp.com/attachments/870706096560562297/887764879774191696/gex_lobby.png');
